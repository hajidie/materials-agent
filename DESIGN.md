...
- EBSD 上传：ChatComposer/useAgentRuns；输入图片来源与预览：EbsdImage，复用 AssetGallery 加载与下载。
...
EBSD 首期为独立单图入口，不使用通用附件数组。上传态、成功预览、移除、失败及原键重试都有可见状态；
上传完成后才能带图提交。草稿按对话和补参目标隔离，网络不确定保留文字、引用和消息原幂等键。
输入图片明确标为用户上传来源，不能显示“生成的 SEM 图像”。移除只解除草稿关联，对话删除清理存储。
EBSD 只展示 Inconel 625、屈服强度/MPa、输入图片、模型来源与实验性限制；不显示延伸率或热处理条件。
性能采用通用 value/unit 结构，界面两位小数，持久化保留原精度；不虚构置信区间。
