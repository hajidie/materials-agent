...
function run(patch: Partial<AgentRun> = {}): AgentRun {
 return {agent_run_id: "run", conversation_id: "conv", source_message_id: "msg", goal: "材料研究", status: "SUCCEEDED", version: 8, waiting_version: 0, waiting: null, pending_execution: null, executions: [], observations: [], steps: [], final_answer: {text: "已完成研究", answer_id: "answer"}, error_code: null, llm_tokens: 100, tool_executions: 0, active_seconds: 1, user_inputs: [], created_at: timestamp, ...patch};
}
...
it("submits a goal and clears its acknowledged draft", async () => {
 ...
 expect(writes[0]?.body).toEqual({mode: "NEW_RUN", content_text: "1000 MPa 转 GPa"}); expect((w.get("textarea").element as HTMLTextAreaElement).value).toBe("");
});
it("resumes the exact Run and waiting version", async () => {
 ...
 expect(writes[0]?.body).toEqual({mode: "RESUME_RUN", content_text: "1000 摄氏度", agent_run_id: "run", waiting_version: 3});
});
it("preserves its key across network uncertainty and reload", async () => {
 ...
 expect(writes[1]?.key).toBe(key); expect(writes[1]?.body).toEqual(writes[0]?.body); expect(sessionStorage.getItem("materials-agent.pending-run.v1")).toBeNull(); expect((reloaded.get("textarea").element as HTMLTextAreaElement).value).toBe("");
});
it("binds confirmation to Invocation and parameter version", async () => {
 items = [run({status: "WAITING_FOR_CONFIRMATION", final_answer: null, waiting_version: 2, pending_execution: {invocation_run_id: "inv", tool_name: "test", status: "PENDING_CONFIRMATION", arguments: {value: 2}, confirmation_version: "hash", confirmation_expires_at: null, retryable: false}})];
