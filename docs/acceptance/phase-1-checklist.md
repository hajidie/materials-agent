...
| M4 | 五类 CHAT_ORCHESTRATION 路径及完整合法 ToolCandidate 的 503 `TOOL_UNAVAILABLE`、零 ToolRun 规则 | 开始 M5 |
| M7 | 完整结果链、selected 引用、稳定 Task 状态和消息 POST 激活点 | 开始 M8 |
...
除上述十个正式里程碑门之外，每个工作单元结束也必须停在项目负责人验收点；这不新增第十一个正式里程碑门。
